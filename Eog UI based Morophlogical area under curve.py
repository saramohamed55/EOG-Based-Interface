...
for signal in removed_DC_signals:
    integration = integrate.simpson(removed_DC_signals[0])
    features_of_signals.append(integration)
...
plt.subplot(122)
plt.fill_between(np.arange(0, len(removed_DC_signals[0])), removed_DC_signals[0], facecolor='r')
plt.title('After Feature Extraction for signal 0')
plt.xlabel('Time')
plt.ylabel('Morophlogical feature area under curve')
plt.show()
